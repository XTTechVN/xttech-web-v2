...
- **Sửa lỗi biên dịch Swift trên Xcode CI/CD ([`NativeTrackingPlugin.swift`](file:///e:/hoc_ve_fullstash/xttech/xttech-web-v2/ios/App/App/NativeTrackingPlugin.swift)):**
  - Cập nhật đúng signature hàm `call.getString(key, defaultValue)` của Capacitor Swift runtime (`JSStringContainer`) với 2 tham số và kiểu trả về non-optional, khắc phục triệt để lỗi build `missing argument for parameter #2 in call` (exit code 65) trên GitHub Actions.
