...
- **Khắc phục lỗi đo sai / thiếu mức Pin trên iOS và một số dòng máy Android ([`NativeTrackingPlugin.swift`](file:///e:/hoc_ve_fullstash/xttech/xttech-web-v2/ios/App/App/NativeTrackingPlugin.swift) & [`TrackingLocationService.java`](file:///e:/hoc_ve_fullstash/xttech/xttech-web-v2/android/app/src/main/java/com/xttech/app/TrackingLocationService.java)):**
  - **Trên iOS:** Kích hoạt `isBatteryMonitoringEnabled = true` ngay từ lúc plugin khởi chạy (`load()`) và đăng ký `NotificationCenter` lắng nghe `batteryLevelDidChangeNotification` để duy trì mức pin mới nhất vào biến cache `lastBatteryLevel`, tránh việc đọc tức thì trả về `-1.0` (unknown).
  - **Trên Android:** Bổ sung truy vấn trực tiếp phần cứng qua `BatteryManager.BATTERY_PROPERTY_CAPACITY` (Android 5.0+), vượt qua rào cản chặn broadcast ngầm của các ROM Xiaomi/Oppo/Vivo khi tắt màn hình, đồng thời lưu cache `lastValidBatteryLevel` làm fallback an toàn.
