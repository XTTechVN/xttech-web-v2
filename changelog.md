...
- Sửa dứt điểm lỗi hiển thị chuỗi văn bản thô (RSC Flight Payload `:HL... 0:{"tree":...}`) trên ứng dụng Android khi đăng xuất hoặc đăng nhập lại:
  - Chuyển đổi toàn bộ cơ chế điều hướng Auth từ soft navigation (`router.push` / `router.replace`) sang hard navigation sạch sẽ (`window.location.replace`) trong [`login-form.tsx`](file:///e:/hoc_ve_fullstash/xttech/xttech-web-v2/src/app/(public)/signin/_components/login-form.tsx), [`signin/page.tsx`](file:///e:/hoc_ve_fullstash/xttech/xttech-web-v2/src/app/(public)/signin/page.tsx), [`AppLauncherRedirect.tsx`](file:///e:/hoc_ve_fullstash/xttech/xttech-web-v2/src/app/_components/AppLauncherRedirect.tsx), [`header-profile.tsx`](file:///e:/hoc_ve_fullstash/xttech/xttech-web-v2/src/components/app-header/header-profile.tsx), [`profile/page.tsx`](file:///e:/hoc_ve_fullstash/xttech/xttech-web-v2/src/app/(auth)/app/(sidebar)/profile/page.tsx) và [`api.ts`](file:///e:/hoc_ve_fullstash/xttech/xttech-web-v2/src/utils/api.ts).
  - Loại bỏ hoàn toàn Race Condition và hiện tượng lưu cache nhầm response `text/x-component` vào WebView History / Back-Forward Cache (bfcache).
  - Bổ sung cấu hình `headers()` trong [`next.config.ts`](file:///e:/hoc_ve_fullstash/xttech/xttech-web-v2/next.config.ts) với `Vary: RSC, Next-Router-State-Tree, Next-Router-Prefetch, Accept` để ngăn chặn reverse proxy / CDN và trình duyệt cache nhầm payload RSC thành HTML.
