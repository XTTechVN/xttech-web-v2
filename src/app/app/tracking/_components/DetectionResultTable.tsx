...
export default function Component({ label, onTrace }: DetectionResultTableProps) {
    ...
    const { data } = useQuery({
        ...
        queryFn: async () => {
            ...
            if (label === 'all') {
                const res = await api.get(`/api/v1/detected-objects/detection-results?offset=${offset}&limit=${limit}`)
                return res.data
            }

            const res = await api.get(`/api/v1/detected-objects/detection-results?label=${label}&offset=${offset}&limit=${limit}`)
            return res.data
        },
