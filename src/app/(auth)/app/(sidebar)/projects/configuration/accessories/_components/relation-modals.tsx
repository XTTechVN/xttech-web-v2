...
import { getDoors, getMaterials } from '@/actions';
import { Modal, Button } from '@/components';
import { formatDoorType, formatMaterialUnit } from '@/types';
import { CheckCircle2, Loader2 } from 'lucide-react';

// ==========================================
// 1. MODAL GÁN BIÊN DẠNG CỬA
// ==========================================
interface AssignDoorsModalProps {
  isOpen: boolean;
  onClose: () => void;
  activeDoorIds: number[];
  onSave: (selectedIds: number[]) => Promise<void>;
  isSaving: boolean;
}

export function AssignDoorsModal({
  isOpen,
  onClose,
  activeDoorIds,
  onSave,
  isSaving,
}: AssignDoorsModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      setSelectedIds(activeDoorIds);
      setSearchTerm('');
    }
  }, [isOpen, activeDoorIds]);

  const { data: allDoors, isLoading } = useQuery({
    queryKey: ['all-doors-assignment'],
    queryFn: () => getDoors({ limit: 9999 }),
    enabled: isOpen,
  });
  ...
  const filteredDoors = allDoors?.items.filter((door) => 
    door.name.toLowerCase().includes(searchTerm.toLowerCase()) ||
    (door.code && door.code.toLowerCase().includes(searchTerm.toLowerCase()))
  ) || [];
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Cấu hình biên dạng cửa áp dụng"
      className="m-2 max-w-2xl w-full"
    >
      <div className="flex flex-col gap-4 pt-2">
        <p className="text-xs text-slate-400">
          Chọn các thiết kế biên dạng cửa sẽ được áp dụng phụ kiện này. Nhấn Xác nhận lưu để cập nhật liên kết.
        </p>
        ...
        <div className="relative">
          <input
            type="text"
            placeholder="Tìm kiếm cửa theo tên hoặc mã..."
            value={searchTerm}
        ...
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-12 text-center">
            ...
            <p className="text-slate-500 text-xs">Đang tải danh sách biên dạng cửa...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 max-h-[360px] overflow-y-auto pr-1">
            {filteredDoors.length > 0 ? (
              filteredDoors.map((door) => {
                const isChecked = selectedIds.includes(door.id);
                return (
                  <label
                    key={door.id}
                  ...
                  >
                    <input
                      ...
                      onChange={(e) => {
                        if (e.target.checked) {
                          setSelectedIds([...selectedIds, door.id]);
                        } else {
                          setSelectedIds(selectedIds.filter((id) => id !== door.id));
                        }
                      }}
                      className="rounded border-slate-300 text-teal-600 focus:ring-teal-500 w-4 h-4 mt-0.5"
                    />
                    <div className="flex flex-col gap-1">
                      <span className="font-semibold text-sm leading-snug text-slate-800">{door.name}</span>
              ...
              <div className="col-span-full py-12 text-center border border-dashed border-slate-200 rounded-xl bg-slate-50/50">
                <p className="text-xs text-slate-400 italic">Không tìm thấy thiết kế cửa nào phù hợp</p>
              </div>
            )}
          </div>
        )}
...
export function AssignMaterialsModal({
  ...
  const { data: allMaterials, isLoading } = useQuery({
    queryKey: ['all-materials-assignment'],
  ...
  return (
    <Modal
      isOpen={isOpen}
      onClose={onClose}
      title="Cấu hình hệ nhôm áp dụng"
      className="m-2 max-w-2xl w-full"
    >
      <div className="flex flex-col gap-4 pt-2">
        <p className="text-xs text-slate-400">
          Chọn các hệ nhôm sẽ được áp dụng phụ kiện này. Nhấn Xác nhận lưu để cập nhật liên kết.
        </p>
